...
import { getMensagens, addMensagem, Mensagem } from './../models/mensagem.model';
...
export const analisaMensagens = async (mensagens: any[], remetente_id: number) => {
    ...
    let mensagem: Mensagem = {
        id: mensagens['messages'][0]['id'],
        numero_cliente: mensagens['number'],
        nome_cliente: mensagens['name'],
        mensagem: mensagens['messages'][0]['message'],
    };

    await addMensagem(mensagem);
    console.log('XXXXXXXXXXXXXXXXXXXXX')
    console.log('XXXXXXXXXXXXXXXXXXXXX')
    console.log('XXXXXXXXXXXXXXXXXXXXX')
    console.log(await getMensagens());
    console.log('XXXXXXXXXXXXXXXXXXXXX')
    console.log('XXXXXXXXXXXXXXXXXXXXX')
    console.log('XXXXXXXXXXXXXXXXXXXXX')

    // let msg = '';
    // if (mensagens[0].number == '558388449964'){
    //     // msg = 'Olá, mary beatrice: ';
    //     msg = 'Olá, ' + mensagens[0].profileName;
    // }
    // if (msg != '')
    //     await remetente.page.evaluate('sendMessageToId("' + mensagens[0].id + '"," '+msg+' ");');
