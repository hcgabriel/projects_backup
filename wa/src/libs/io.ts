...
//asçldkasçlkdçalskdçlaskdçlaskdçlask
...
app.get('/', (req, res) => {
  console.log('klqjsdlkajsdklajs')
});

app.get('/api/mensagens', async (req, res) => {
    return res.send(await getMensagens());
});
...
io.on('connection', function (socket: any) {
  ...
  socket.on('lerMensagens', async () => {
    socket.emit('mensagens', await getMensagens());
  });
